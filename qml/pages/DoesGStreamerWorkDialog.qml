...
Dialog {
    ...
    Column {
        ...
        Label {
            ...
            text: qsTr("If the torch is on, then the new GStreamer implementation works on your device - click YES to use it instead of the old controlfile method. If the torch is not on, click NO to use the controlfile method.")
        }
    }

    onDone: {
        if (result == DialogResult.Accepted) {
            led.setUseGStreamer(true)
            led.setGSTDialogCompleted()
        } else if (result == DialogResult.Rejected) {
            led.setUseGStreamer(false)
            led.setGSTDialogCompleted()
        }
    }
}
